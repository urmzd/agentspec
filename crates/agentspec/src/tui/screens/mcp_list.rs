...
pub fn draw(f: &mut Frame, area: Rect, app: &App) {
    ...
    let rows: Vec<Row> = servers
        .iter()
        .map(|s| {
            let dots = tool_dots(&app.mcp_tools, &s.linked_tools);

            Row::new(vec![
                Cell::from(s.name.clone()).style(Style::default().fg(Color::Green)),
                Cell::from(s.transport.clone()).style(Style::default().fg(Color::DarkGray)),
                Cell::from(truncate(&s.summary, 44)),
                Cell::from(dots),
            ])
        })
        .collect();
...
fn tool_dots(mcp_tools: &[String], linked: &[String]) -> String {
    mcp_tools
        .iter()
