...
pub fn refresh_cache_with_root(
    ...
    let discovered = discover_unmanaged(cfg, broad_root, extra_paths)?;
...
pub fn adopt_all(cfg: &mut Config, all_tools: bool, copy: bool) -> Result<()> {
    ...
    for (name, kind) in &to_adopt {
        if let Err(e) = adopt(cfg, name, *kind, tool_slugs.as_deref(), all_tools, copy) {
            eprintln!(
                "  {} Could not adopt {} '{}': {e}",
                style("✗").red(),
                kind,
...
pub fn status(cfg: &Config, json: bool) -> Result<()> {
    if json {
        let managed: Vec<serde_json::Value> = cfg
            .resources
            .iter()
            .map(|r| {
                serde_json::json!({
