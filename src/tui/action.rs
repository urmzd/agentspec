...
impl Action {
    pub fn execute(&self) -> Result<()> {
        match self {
            Action::DeleteSkill(name) => remove_ops::remove_skill(name),
            Action::DeleteAgent { name, source } => match source {
                AgentSource::Managed => remove_ops::remove_agent(name),
            ...
            Action::Link {
                ...
                name,
                tool,
            } => link_ops::link(*kind, name, tool, false),
            Action::Unlink {
                kind,
                name,
                tool,
            } => link_ops::unlink(*kind, name, tool),
        }
    }
