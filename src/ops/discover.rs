...
pub fn scan(json: bool) -> Result<()> {
    ...
    // Update config cache
    let mut cfg = inventory::load_config()?;
    cfg.discovered = discovered.clone();
    cfg.last_scan = Some(chrono::Utc::now().to_rfc3339());
    inventory::save_config(&cfg)?;

    // Scan project configs (AGENTS.md + memory counts)
    let project_infos = memory::scan_project_infos();

    if json {
        let unmanaged: Vec<serde_json::Value> = discovered
            .iter()
            .map(|d| {
                serde_json::json!({
                    "name": d.name,
                    "kind": format!("{}", d.kind),
                    "found_in": d.found_in.iter().map(|l| {
                        serde_json::json!({ "tool": l.tool, "path": l.path })
                    }).collect::<Vec<_>>(),
                })
            })
            .collect();
    ...
    if discovered.is_empty() {
        ...
        println!(
            ...
            style("Found In").bold().underlined()
        );

        for d in &discovered {
            let tools_str: Vec<String> = d.found_in.iter().map(|l| l.tool.clone()).collect();
            println!(
                "  {:<25} {:<10} {}",
                style(&d.name).cyan(),
                format!("{}", d.kind),
        ...
        println!(
            "\n  {} unmanaged resource(s) found. Use `agentspec manage add <name>` or `agentspec manage all` to bring under management.",
