...
#   AGENTSPEC_VERSION     — version to install (e.g. "v1.2.0"); defaults to latest
#   AGENTSPEC_INSTALL_DIR — installation directory; defaults to $HOME/.local/bin
#   AGENTSPEC_SHA256      — expected SHA256 checksum of the binary (hex string); skips verification if unset
...
main() {
    ...
    if [ -n "${AGENTSPEC_SHA256:-}" ]; then
        if command -v sha256sum >/dev/null 2>&1; then
            actual=$(sha256sum "$install_dir/agentspec" | awk '{print $1}')
        elif command -v shasum >/dev/null 2>&1; then
            actual=$(shasum -a 256 "$install_dir/agentspec" | awk '{print $1}')
        else
            err "sha256sum or shasum required for checksum verification"
        fi
        if [ "$actual" != "$AGENTSPEC_SHA256" ]; then
            rm -f "$install_dir/agentspec"
            err "SHA256 mismatch: expected $AGENTSPEC_SHA256, got $actual"
        fi
...
add_to_path() {
    ...
    if [ "$(basename "$SHELL")" = "fish" ]; then
        if ! grep -q "$install_dir" "$profile" 2>/dev/null; then
            ...
            echo "" >> "$profile"
            echo "# Added by agentspec installer" >> "$profile"
